...
local c={gpu=gpu,width=w,height=h,KEY_UP=200,KEY_DOWN=208,KEY_ENTER=28,KEY_BACKSPACE=14,KEY_END=207}
...
function c.message(title,text)
 ...
 while true do local e={computer.pullSignal()}; if e[1]=="touch" then return end; if e[1]=="key_down" and ((e[4] or 0)==c.KEY_END or (e[4] or 0)==c.KEY_BACKSPACE or (e[4] or 0)==c.KEY_ENTER or (e[3] or 0)==8) then return end end
end
function c.menu(title,sub,items,selected)
 ...
 while true do
  ...
  gpu.setBackground(0x0B0B0B); c.footer("UP/DOWN + ENTER   END = back   mouse/touch supported")
  local e={computer.pullSignal()}
  if e[1]=="key_down" then local ch,code=e[3] or 0,e[4] or 0; if code==c.KEY_UP then selected=selected-1; if selected<1 then selected=#items end elseif code==c.KEY_DOWN then selected=selected+1; if selected>#items then selected=1 end elseif code==c.KEY_ENTER or ch==13 then return selected elseif code==c.KEY_END or code==c.KEY_BACKSPACE or ch==8 then return nil end
...
function c.input(title,prompt,initial)
 ...
 while true do
  c.header(title,prompt); gpu.setBackground(0x303030); gpu.setForeground(0xFFFFFF); gpu.fill(4,8,w-7,1," "); gpu.set(6,8,v:sub(math.max(1,#v-w+12))); c.footer("ENTER confirm   Ctrl+V paste   END cancel")
  local e={computer.pullSignal()}
  if e[1]=="clipboard" then v=v..tostring(e[3] or ""):gsub("[\r\n]","")
  elseif e[1]=="key_down" then local ch,code=e[3] or 0,e[4] or 0; if code==c.KEY_ENTER or ch==13 then return v elseif code==c.KEY_END then return nil elseif code==c.KEY_BACKSPACE or ch==8 then v=v:sub(1,-2) elseif ch>=32 and ch<=126 then v=v..string.char(ch) end end
 end
end
YellowOS.common=c
