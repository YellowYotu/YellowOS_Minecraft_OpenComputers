...
local function loadFile(path)
    ...
    local data = ""
    while true do local chunk = fs.read(handle, 2048); if not chunk then break end; data = data .. chunk end
    fs.close(handle)
...
_G.YellowOS = {fs=fs,loadFile=loadFile,version="0.2.4",edition="Server Edition"}
