...
local function draw(prompt)
    ...
    gpu.set(3, 4, "Type 'help' for commands. BACKSPACE on empty line exits.")
    ...
    gpu.set(3, height - 2, "> " .. prompt:sub(1, width - 6))
end
...
local function installUpdate()
    ...
    if not manifest then
        printLine("Update check failed: " .. tostring(reason))
        return
    end
    ...
    if not manifest.updateAvailable then
        printLine("Server is already up to date.")
        return
    end
    ...
    if not ok then
        printLine("Update failed: " .. tostring(installReason))
        return
    end
...
local function execute(command)
    ...
    elseif command == "help" then
        ...
        printLine("update   - update Server Edition now")
        printLine("version  - show YellowOS version")
        printLine("clear    - clear terminal")
        printLine("reboot   - reboot server")
        printLine("shutdown - shut down server")
        printLine("exit     - return to Control Center")
    elseif command == "update" then
        installUpdate()
    elseif command == "version" then
        printLine("YellowOS Server Edition " .. YellowOS.version)
...
while true do
    ...
    if signal == "key_down" then
        if code == common.KEY_ENTER then
            printLine("> " .. input)

            if execute(input) then
                return
            end
