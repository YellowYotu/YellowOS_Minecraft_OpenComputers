...
local function draw()
    common.header("Terminal", "Protected system disk | Ctrl+V paste | END exit")
    ...
    gpu.set(3, common.height - 1, "> " .. input:sub(math.max(1, #input - common.width + 6)))
end
...
local function list(path)
    ...
    if not fs.exists(path) or not fs.isDirectory(path) then push("Not a directory: " .. path); return end
    local names = {}
...
local function cat(path)
    path = normalizePath(path)
    if not fs.exists(path) or fs.isDirectory(path) then push("Not a file: " .. path); return end
    local handle = fs.open(path, "r")
    if not handle then push("Cannot open file"); return end
    local data = ""
    while #data < 4096 do
...
local function doUpdate()
    ...
    if not manifest then push("Check failed: " .. tostring(reason)); return end
    if not manifest.updateAvailable then push("Already up to date: " .. YellowOS.version); return end
    push("Installing " .. manifest.version .. "...")
    draw()
    local ok, installReason = updater.install(manifest)
    if not ok then push("Update failed: " .. tostring(installReason)); return end
...
local blocked = {rm=true,del=true,delete=true,edit=true,write=true,mv=true,move=true,cp=true,copy=true,mkdir=true,touch=true,wget=true,lua=true}

local function execute(commandLine)
    local command, args = commandLine:match("^(%S+)%s*(.-)%s*$")
    command = (command or ""):lower()
    if command == "" then return
    elseif command == "help" then push("help version device ls [path] cat <file> update clear reboot shutdown exit")
    elseif command == "version" then push("YellowOS " .. YellowOS.version .. " - " .. YellowOS.edition)
    elseif command == "device" then push(YellowOS.device)
    elseif command == "ls" then list(args ~= "" and args or "/")
    elseif command == "cat" then if args == "" then push("Usage: cat <file>") else cat(args) end
    elseif command == "update" then doUpdate()
    elseif command == "clear" then lines = {}
    elseif command == "reboot" then computer.shutdown(true)
    elseif command == "shutdown" then computer.shutdown(false)
    elseif command == "exit" then running = false
    elseif blocked[command] then push("Denied: device system disk is protected.")
    else push("Unknown command: " .. command) end
end
...
while running do
    ...
    if signal == "clipboard" then
        input = input .. tostring(event[3] or ""):gsub("[\r\n]", "")
    ...
    elseif signal == "touch" then
        ...
        if y <= 3 and x <= 12 then running = false end
    else
